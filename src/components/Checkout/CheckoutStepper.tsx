...
export const CheckoutStepper = () => {
    const { thirdStep, handleThirdStep, secondStep, handleSecondStep } =
        useCheckout()
    const { cartQuantity } = useShoppingCart()
    const [showLabels, setShowLabels] = useState(true)
    const { width } = useViewportSize()
    const [active, setActive] = useState(0)
    const [isFirstStepValidate, setIsFirstStepValidate] = useState(
        cartQuantity > 0
    )
    ...
    return (
        <div className={classes.wrapper}>
            <Group position="center">
                ...
                >
                    <Stepper.Step
                        icon={<IconShoppingCart color={theme.colors.blue[7]} />}
                    ...
                    <Stepper.Step
                        icon={<IconUserCheck color={theme.colors.blue[7]} />}
                    ...
                    <Stepper.Step
                        icon={<IconShieldCheck color={theme.colors.blue[7]} />}
            ...
            <Group className={classes.navButtons} mb={theme.spacing.xl}>
                ...
                {active === 0 ? (
                    <Button
                        size={'lg'}
                        onClick={nextStep}
                        disabled={!isFirstStepValidate}
                    >
                        Dalej
                    </Button>
                ) : active === 1 ? (
                    <Button size={'lg'} onClick={validateSecondStep}>
                        Dalej
                    </Button>
                ) : active === 2 ? (
                    <Button size={'lg'} onClick={validateThirdStep}>
                        Dalej
                    </Button>
                ) : active === 3 ? (
                    ''
