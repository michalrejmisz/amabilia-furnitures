...
export const FinalStep = () => {
    const { thirdStep, handleThirdStep, handleFormProps, formProps } =
        useCheckout()
    ...
    useEffect(() => {
        if (isComment === false) {
            handleFormProps({ moreInformation: '' })
        }
    }, [isComment])

    const handlePrivacyAgreement = (value) => {
        if (value == 'tak') {
            handleThirdStep({ isClicked: thirdStep.isClicked, isValid: true })
        }
        if (value == 'nie') {
            handleThirdStep({ isClicked: thirdStep.isClicked, isValid: false })
        }
    ...
    const alertStyle =
        thirdStep.isValid == false && thirdStep.isClicked == true
        ...
        <>
            ...
            <form className={classes.form}>
                <Alert className={classes.alert}>
                    <p style={{ color: theme.colors.blue[6] }}>
                        Wybierz w jaki sposób ma się z Tobą skontaktować nasz
                        pracownik w celu finalizacji zakupu.
                    </p>
                </Alert>
                <SegmentedControl
            ...
            <form className={classes.form}>
                <Alert
                    ...
                    title="Zgoda na newsletter(opcjonalnie)."
                >
                    <p style={{ color: theme.colors.blue[6] }}>
                        Wyrażam zgodę na przesyłanie mi za pomocą środków
                        komunikacji elektronicznej informacji handlowej w
                        postaci Newslettera przez lub na zlecenie Castorama
                        Polska Sp. z o.o., z siedzibą w Warszawie (02-255), przy
                        ul. Krakowiaków 78, wpisaną do rejestru przedsiębiorców
                        Krajowego Rejestru Sądowego prowadzonego przez Sąd
                        Rejonowy dla m. st. Warszawy w Warszawie, XIII Wydział
                        Gospodarczy Krajowego Rejestru Sądowego, pod numerem KRS
                        0000024785, w rozumieniu ustawy z dnia 18 lipca 2002 r.
                ...
                <Alert
                    className={classes.alert}
                    style={alertStyle}
                    title="Wymagana akceptacja regulaminów."
                    color={`${thirdStep.isValid == false && thirdStep.isClicked == true ? 'red' : 'blue'}`}
                >
                    <PrivacyPolicyModal>
                        <p style={alertStyle}>
                            Oświadczam, iż zapoznałem się z{' '}
                            <u>Polityką Cookies </u>oraz{' '}
                            <u>Polityką Prywatności</u> i je akceptuję.
                        </p>
                    </PrivacyPolicyModal>
                </Alert>
                <SegmentedControl
            ...
            <form className={classes.form}>
                <Group
                    mb={theme.spacing.xl}
                    style={{ width: '100%', color: theme.colors.blue[6] }}
                >
                    {!isComment && (
                        <UnstyledButton onClick={handleShowCommentArea}>
                            <Group style={{ color: theme.colors.blue[6] }}>
                                <IconPlus />
                                <Title order={6}>
                                    Dodaj uwagi do zamówienia(opcjonalnie)
                                </Title>
                            </Group>
                        </UnstyledButton>
                    )}
