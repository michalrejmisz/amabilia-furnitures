...
export const Completed = () => {
    ...
    const submitOrder2 = async() => {
        ...
        // await fetch('http://localhost:8082/api/oders', {
            method: 'POST',
            headers: {
                ...
                Miejscowosc: formProps.isDelivery ? formProps.Miejscowosc : '',
                KodPocztowy: formProps.isDelivery ? formProps.KodPocztowy : '',
                Street: formProps.isDelivery ? formProps.Street : '',
                houseNumber: formProps.isDelivery ? formProps.houseNumber : '',
    ...
    return(
        <>
            <Alert className={classes.alert} mt={theme.spacing.xl} style={{minHeight: "200px", justifyContent: "center", textAlign: "center", display: "flex", alignItems: "center"}}>
                {/*<Button onClick={submitOrder2}></Button>*/}
                    {isLoading ? (
                            <Loader color="blue"/>
                    ) : isError ? (
                        <>
                            <Title order={1} style={{color: theme.colors.blue[6]}}>Niestety wystąpił błąd!</Title>
                            <Text size={"xl"} style={{color: theme.colors.blue[6]}}>Spróbuj ponownie za kilka minut</Text>
                        </>
                    ) : (
                        <>
