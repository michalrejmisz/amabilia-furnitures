...
const CustomerForm = () =>  {
    ...
    const {handleSecondStepValid, isSecondStepClicked, handleSecondStep, secondStep, formProps, handleFormProps} = useCheckout();
    const userForm = useForm({
        ...
        validate: {
            firstName: (value) => (value.length < 2 ? 'Imię musi posiadać przynajmniej dwa znaki' : null),
            secondName: (value) => (value.length < 2 ? 'Nazwisko musi posiadać przynajmniej dwa znaki' : null),
            email: isEmail('Wprowadź poprawny email'),
            phoneNumber: (value) => (value.length < 9  ? 'Wprowadź poprawny numer telefonu' : null),
        },
        validateInputOnChange: ['firstName', 'secondName', 'email', 'phoneNumber'],
    });
    ...
    const receiptForm = useForm({
        ...
        validate: {
            nip: (value) => (value.length != 10 ? "NIP składa się z 10 cyfr" : null),
        },
    ...
    const deliveryForm = useForm({
        ...
        validate: {
            city: (value) => (value.length < 1 ? 'Pole nie może być puste' : null),
            postalCode: (value) => (value.length < 1 ? 'Pole nie może być puste' : null),
            street: (value) => (value.length < 1 ? 'Pole nie może być puste' : null),
            houseNumber: (value) => (value.length < 1 ? 'Pole nie może być puste' : null),
        },
        validateInputOnChange: ['firstName', 'postalCode', 'street', 'houseNumber'],
    });
            ...
            if(segmentControlReceipt === "true"){
                if(receiptForm.isValid()){
                    handleFormProps({
                        ...
                        NazwaFirmy: receiptForm.getInputProps('companyName').value,
                    })
                }
                if(segmentControlDelivery === "true"){
                    handleFormProps({
                        ...
                        KodPocztowy: deliveryForm.getInputProps('postalCode').value,
                        houseNumber: deliveryForm.getInputProps('houseNumber').value
                    })
                    if(receiptForm.isValid() && deliveryForm.isValid()){handleSecondStep({...secondStep, isValid: true})} else {handleSecondStep({...secondStep, isValid: false})}
                } else {
                    if(receiptForm.isValid()){handleSecondStep({...secondStep, isValid: true})} else {handleSecondStep({...secondStep, isValid: false})}
                }
            } else {
                if(segmentControlDelivery === "true"){
                    ...
                    } else {handleSecondStep({...secondStep, isValid: false})}
                } else {
                    handleSecondStep({...secondStep, isValid: true})
                }
            }
    ...
    }, [secondStep.isUserValid, secondStep.isReceiptValid, secondStep.isDeliveryValid, segmentControlReceipt, segmentControlDelivery, formChange])
        ...
        <div style={{float: "left"}}>
            ...
            <form className={classes.form} onChange={handleFormChange}>
                ...
                <TextInput
                    ...
                    // onBlur={handleFormProps({...formProps, Nazwisko: userForm.getInputProps('secondName').value})}
                />
                ...
                <TextInput
                    ...
                    // onBlur={handleFormProps({...formProps, Email: userForm.getInputProps('email').value})}
                />
                ...
                <TextInput
                    ...
                    // onBlur={handleFormProps({...formProps, Telefon: userForm.getInputProps('phoneNumber').value})}
                />
            </form>
            ...
            <form className={classes.form}>
                ...
                <Transition mounted={mountReceipt} transition="scale-y" duration={400} timingFunction="ease">
                    {(styles) => (
                        <form className={classes.inputsGroup} onChange={handleFormChange}>
                            <TextInput
                                className={classes.item}
                                style={{...styles}}
                    ...
                    {(styles) => (
                        <>
                            <Alert icon={<IconAlertCircle size={24}/>} title="Uwaga!" className={classes.alert} style={{...styles}}>
                                ...
                                {data?.zamowienieDostawa?.data?.attributes?.InformacjaMyslnik?.map((item, index) => {
                                    return(
                                        <p key={index} style={{color: theme.colors.blue[6]}}>{item.Myslnik}</p>
                                    )
                                })}
                            ...
                            <form className={classes.inputsGroup} onChange={handleFormChange}>
                                ...
                                <TextInput
                                    ...
                                    withAsterisk
                                    placeholder="60-123"
                                    {...deliveryForm.getInputProps('postalCode')}
                                    className={classes.item}
                                ...
                                <TextInput
                                    ...
                                    withAsterisk
                                    placeholder="171/12b"
                                    {...deliveryForm.getInputProps('houseNumber')}
                                    className={classes.item}
