...
const ProductsGrid = ({data, handleSearchBar}: Props) => {
    ...
    return(
        <Grid style={{marginBottom: "30px"}}>
            <Grid.Col span={12}><SearchBar handleSearchInput={handleSearchBar}/></Grid.Col>
            {data?.map((product: { attributes: { Nazwa: string, Cena: number, Link: string, Opis: string, Wymiary: string, Zdjecie_glowne?: { data?: { attributes?: { url: string } } } } }) => (
                                       ...
                                       product={{
                                           ...
                                           imagePrimary: {
                                               ...
                                               thumbnailUrl: `${
                                                   ...
                                                   width > 575 ? (
                                                       product?.attributes?.Zdjecie_glowne?.data?.attributes?.formats?.thumbnail?.url
                                                           ? product?.attributes?.Zdjecie_glowne?.data?.attributes?.formats?.thumbnail?.url+"?format=webp"
                                                           : product?.attributes?.Zdjecie_glowne?.data?.attributes?.url
                                                           || "/uploads/no-thumb.png"
                                                   ) : (
                                                           product?.attributes?.Zdjecie_glowne?.data?.attributes?.formats?.small?.url
                                                           ? product?.attributes?.Zdjecie_glowne?.data?.attributes?.formats?.small?.url+"?format=webp"
                                                           : product?.attributes?.Zdjecie_glowne?.data?.attributes?.url
                                                           || "/uploads/no-thumb.png"
                                                    )}`,
                                           },
