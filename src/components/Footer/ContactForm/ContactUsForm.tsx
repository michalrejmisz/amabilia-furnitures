import {
    ...
    LoadingOverlay,
    Loader,
    Container, Checkbox, useMantineTheme, Modal,
} from '@mantine/core';
...
export function ContactUsForm() {
    ...
    const contactForm = useForm({
        ...
        validate: {
            ...
            message: (value) => (value.length < 1 ? 'Wiadomość jest pusta!' : null),
    ...
    const handleSubmit = async() => {
        if(contactForm.isValid()){
            ...
            const response = await axios.post('https://creator.amabilia-meble.pl/api/ezforms/submit', {token, formData: mailForm})
            .then((res) => {
                setIsLoading(false)
                contactForm.reset();
            })
    ...
    const termsOfServiceText = <PrivacyPolicyModal><a><u>Wyrażam zgodę na przetwarzanie danych osobowych.</u></a></PrivacyPolicyModal>
            ...
            <div className={classes.wrapper}>
                ...
                >
                    {isLoading ? (
                        ...
                        <Title order={5} style={{margin: "30px", color: theme.white}}>Niestety wystąpiły problemy z uwierzytelnieniem. Prosimy spóbować ponownie później! </Title>
                    ): (
                        <Title order={5} style={{margin: "30px", color: theme.white}}>Dziękujemy! W krótce się z Tobą skontaktujemy.</Title>
                    )}
                </Modal>
                <Container>
                    <SimpleGrid cols={2} spacing={50} breakpoints={[{ maxWidth: 'sm', cols: 1 }]}>
                        <div>
                            ...
                            <Text className={classes.description} mt="sm" mb={30}>
                                Zostaw nam wiadomość, a odpiszemy w ciągu 24 godzin roboczych.
                            </Text>

                            <ContactIconsList variant="white" />

                        </div>
                        <form onSubmit={contactForm.onSubmit(handleSubmit)} className={classes.form}>
                            <TextInput
                                styles={{
                                    error: {
                                        textAlign: "left",
                            ...
                            <Group position="right" mt="md">
                                <Button type="submit" className={classes.control}>Wyślij</Button>
                            </Group>
                        </form>
                    </SimpleGrid>
                </Container>
