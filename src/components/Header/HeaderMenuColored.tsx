import { createStyles, Header, Menu, Group, Center, Burger, Container, Title, UnstyledButton } from '@mantine/core';
...
export function HeaderMenuColored({ links, transparent, onClickCart, onClickToFooter}: HeaderSearchProps ) {
    ...
    if(pathname.indexOf('/products/') === 0 || pathname.indexOf('/products') === 0 ||
        ...
        pathname.indexOf('/about/') === 0 || pathname.indexOf('/about') === 0 ||
        pathname.indexOf('/checkout') === 0 || pathname.indexOf('/checkout/') === 0
    ){
        transparent = false;
    }
        ...
        return (
                ...
                typeof link.link === 'string' ? (
                    <Link href={{pathname: link.link}} key={link.link} legacyBehavior>
                        <a className={classes.link} key={link.link}>{link.label}</a>
                    </Link>
                ) : (
                    <UnstyledButton key={link.link}>
                        <a className={classes.link} key={link.link} onClick={link.link}>{link.label}</a>
                    </UnstyledButton>
                )
    )});
