...
const DescriptionElement = ({
    ...
    let polygon
    if (imageSite == 'lewa') {
        polygon = 'polygon(0% 0%, 75% 0, 100% 100%, 0% 100%)'
    }
    if (imageSite == 'prawa') {
            ...
            <Grid columns={12} className={classes.grid}>
                <>
                    <ImageCol
                        polygon={polygon}
                        image={image}
                        order={imageSite === 'lewa' ? 1 : 2}
                    />
                    <DescriptionCol
                        title={title}
                        subtitle={subtitle}
                        order={imageSite === 'lewa' ? 2 : 1}
                    />
                </>
            </Grid>
        </>
...
const About: NextPageWithLayout = () => {
    ...
    return (
        <>
            <NextSeo title="Informacje o sklepie | Amabilia-meble.pl " />
            <Container size="lg" style={{ paddingBottom: 96 }}>
                ...
                >
                    <div style={{ flex: 1 }}>
                        ...
                        >
                            {data?.aboutus?.data?.attributes?.Tytul ??
                                'Witajcie w sklepie Amabilia'}
                        </Title>
                        <Text
                            size="lg"
                            style={{
                        ...
                        >
                            {data?.aboutus?.data?.attributes?.Podtytul ??
                ...
                {data?.aboutus?.data?.attributes?.NowyElementZeZdjeciem?.map(
                    (item, index) => (
                        <DescriptionElement
                            key={index}
